...
1.从晴天钟获取气象预报(输入地名或上传位置)
2.查询全天88星座(如:白羊座)
3.查询超过3万个深空天体的资料(如:M42, NGC7000, 加州星云)
若想获取详细帮助, 请回复对应数字.'''
...
command_dicts = defaultdict(lambda: u'找不到这个指令哦', {
    ...
    '2': u'直接输入星座名称就可以啦, 比如"白羊座", 省略一点写"白羊"也行. 当然也可以用三字缩写, 如Ari.',
    '3': u'想在30000+深空天体中漫游么? 你可以输入编号查询, 如:M42, C4, NGC7000; 也可以使用名称, 如"仙女座大星系", "加州星云"',
    '0': u'呵呵'
})
