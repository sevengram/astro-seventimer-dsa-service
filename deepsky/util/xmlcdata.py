...
def seventimer_response(touser, lat, lon, label):
    ...
    response['Articles']['item'].append(
        {'Title': cdata(label), 'Description': cdata(u'数据来自晴天钟(7timer.com)'), 'PicUrl': cdata(image_url),
         'Url': cdata(image_url)})
    result = xmltodict.unparse({'xml': response})
    return special_decode(result[result.index('\n') + 1:])
...
def constellation_response(touser, result):
    ...
    response['Articles']['item'].append(
        {'Title': cdata(title), 'Description': cdata(article), 'PicUrl': cdata(result['image']),
         'Url': cdata(result['wiki'])})
    result = xmltodict.unparse({'xml': response})
    return special_decode(result[result.index('\n') + 1:])
...
def solar_response(touser, result, chinese):
    ...
    response['Articles']['item'].append(
        {'Title': cdata(result['chinese']), 'Description': cdata(article), 'PicUrl': cdata(image_url),
         'Url': cdata(target_url)})
    result = xmltodict.unparse({'xml': response})
    return special_decode(result[result.index('\n') + 1:])
...
def deepsky_response(touser, info, chinese):
    ...
    objname = info['object']
    title = objname + ' - %s' % info['cn_name'] if 'cn_name' in info else objname
